...
""" Python implementation of VFH+ algorithm
    ...
    Implementing binary ph and masked ph to account for steering
...
class PolarHistogram:
    ...
    def __init__(self, nsectors):
        ...
        self.state = PolarHistogram.PH
        self.avg_obs_dis = None

    def get(self, index):
        return self.polar_histogram[bound(index, self.polar_histogram)]
    ...
    def binary_polar_histogram(self, t_low, t_high):
        ...
        binary_ph = [0] * self.nsectors

        for i in xrange(len(self.polar_histogram)):
            c = self.polar_histogram[i]
            if c > t_high:
                binary_ph[i] = 1
            elif c < t_low:
                binary_ph[i] = 0
...
class VFH:
    ...
    def map_active_hg_to_ph(cls, hg, ph, vcp, w_s, a=1, b=1, l=5):
        ...
        # ar_string = ""
        # for y in xrange(hg.get_valid_y(robot_y - window_radius), hg.get_valid_y(robot_y + window_radius + 1)):
        ...
        # Deprecated in vfh+ due to obstacle enlargement
    ...
    def get_best_direction(cls, ph, target_direction, current_direction, previous_direction, t_low=25, t_high=30, smax=5, a=5, b=1, c=1):
        ...
        ph.binary_polar_histogram(t_low, t_high)
        print "Binary Polar Histogram"
        print ph
        ph = ph.polar_histogram

        if len(filter(lambda x: x == 1, ph)) == 0:
        ...
        # for v in free_valleys:
        #     if v[1] - v[0] > smax * sector_angle: # wide valley
        #         left_candidate = v[0] + smax * sector_angle / 2
        #         right_candidate = v[1] - smax * sector_angle / 2
        #         candidate_angles.append(wrap_angle(left_candidate)) # Add start and end to candidate angles
        #         candidate_angles.append(wrap_angle(right_candidate))
        #         print "Wide Valley (%d, %d)... Adding %d and %d" % (v[0], v[1], candidate_angles[-2], candidate_angles[-1])

        #         if left_candidate < target_direction < right_candidate or left_candidate < target_direction + 360 < right_candidate: # If target angle is in a free valley, add it to candidate angles
        #             print "Target in btwn (%d, %d)... Adding %d" % (left_candidate, right_candidate, target_direction)
        #             candidate_angles.append(target_direction)
