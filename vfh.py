...
class VFH:
    ...
    def map_active_hg_to_ph(cls, hg, ph, vcp, w_s, a=1, b=1, l=5):
        ...
        for x in xrange(robot_x - window_radius, robot_x + window_radius + 1):
            for y in xrange(robot_y - window_radius, robot_y + window_radius + 1):
                ...
                for sector_index in xrange(int(math.floor((cell_angle - layer_angle_interval / 2) / ph.sector_angle)), \
    ...
    def get_best_direction(cls, ph, target_direction, current_direction, previous_direction, t=15, smax=5, a=5, b=1, c=1):
        ...
        for v in free_valleys:
            if v[1] - v[0] > smax * sector_angle: # wide valley
                candidate_angles.append(wrap_angle(v[0] + smax * sector_angle / 2)) # Add start and end to candidate angles
                candidate_angles.append(wrap_angle(v[1] - sector_angle * sector_angle / 2))
                print "Wide Valley (%d, %d)... Adding %d and %d" % (v[0], v[1], candidate_angles[-2], candidate_angles[-1])

                if v[0] < target_direction < v[1] or v[0] < target_direction + 360 < v[1]: # If target angle is in a free valley, add it to candidate angles
                    print "Target in btwn (%d, %d)... Adding %d" % (v[0], v[1], target_direction)
                    candidate_angles.append(target_direction)
                ...
                candidate_angles.append((v[0] + v[1]) / 2) # Add middle of valley to candidat angles
